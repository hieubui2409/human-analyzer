...
[Skill catalog (58)](#skill-catalog--58-skills) ·
...
content recalibration. **58 framework skills** across **6 frameworks**, invoked as `{framework}:{skill}`
...
[catalog below](#skill-catalog--58-skills).
...
flowchart TD
    ...
    S1 --> S2["pytest tests/ -q<br/>→ 698 pass, 34 skipped (live-LLM, key-gated)"]
    S2 --> S3["eval/run_evals.py<br/>→ 4/4 golden cases"]
    S3 --> S4["e2e/run-full-pipeline.py<br/>→ 16/16 deterministic steps"]
...
# example: score a character's profile completeness
.claude/skills/.venv/bin/python3 \
  .claude/skills/psy-health-check/scripts/score-profile-completeness.py --character test-alpha
```
...
```mermaid
flowchart LR
    SRC["Toolkit source<br/>.claude/skills · scripts · schemas<br/>docs/rules · docs/references · e2e"] --> MAN["pack.manifest.yaml<br/>include globs"]
...
```bash
.claude/skills/.venv/bin/python3 tools/pack/build_pack.py          # → dist/ tarball + MANIFEST
...
```bash
.claude/skills/.venv/bin/python3 -m pytest tests/ -q            # full suite (698 pass)
...
| [`frameworks-pack-release.yml`](./.github/workflows/frameworks-pack-release.yml) | tag-triggered reproducible pack build + SHA verify |
...
| Applied-pattern ledger (cleanmatic→here) | [`STANDARDIZE.md`](./STANDARDIZE.md) |
...
Tất cả **theo sự kiện**: thêm tư liệu → làm mới hồ sơ → hiệu chỉnh nội dung. **58 skill** trên **6 framework**,
...
flowchart TD
    ...
    S1 --> S2["pytest tests/ -q<br/>→ 698 đạt, 34 bỏ qua (LLM thật, cần key)"]
...
- **CRE — Nội dung (9):** chuyển hồ sơ thành nội dung gốc nền tảng, kiểm soát theo bậc bằng chứng + bảo mật trước khi đăng. Quy tắc [03](./docs/rules/03-content-creation-pipeline.md) · [09](./docs/rules/09-confidentiality-protocol.md) · [14](./docs/rules/14-cre-evidence-and-events.md).
...
Bảng tra cứu đầy đủ + deep-link song ngữ nằm ở [phần tiếng Anh](#skill-catalog--58-skills) (ID skill, tên flag,
...
```bash
.claude/skills/.venv/bin/python3 tools/pack/build_pack.py          # → tarball + MANIFEST trong dist/
.claude/skills/.venv/bin/python3 -m pytest tests/test_pack_determinism_and_safety.py -q
```
...
```bash
.claude/skills/.venv/bin/python3 -m pytest tests/ -q            # toàn bộ (698 đạt)
