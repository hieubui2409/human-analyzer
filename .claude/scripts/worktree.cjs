...
if (nodeVersion < MIN_NODE_VERSION) {
  outputError(
    "NODE_VERSION_ERROR",
    `Node.js ${MIN_NODE_VERSION}+ required. Current: ${process.version}`,
  );
  process.exit(1);
}
  ...
  if (jsonOutput) {
    ...
    if (data.success) {
      ...
      if (data.envTemplatesCopied && data.envTemplatesCopied.length > 0) {
        ...
        data.envTemplatesCopied.forEach((t) =>
...
function checkGitVersion() {
  ...
  if (!result.success && result.stderr.includes("not a git command")) {
    outputError(
      "GIT_VERSION_ERROR",
      "Git version too old (worktree requires git 2.5+)",
      {
...
function findTopmostSuperproject(gitRoot) {
  ...
  while (depth < MAX_SUPERPROJECT_DEPTH) {
    const result = git("rev-parse --show-superproject-working-tree", {
      silent: true,
      cwd: current,
    });
...
function validateWorktreeRoot(rootPath) {
  ...
  if (fs.existsSync(resolved)) {
    ...
    if (!stat.isDirectory()) {
      return {
        valid: false,
        error: `Path exists but is not a directory: ${resolved}`,
      };
    }
  ...
  return {
    valid: false,
    error: `Cannot create worktree directory: parent path does not exist: ${parent}`,
  };
}
  ...
  if (explicitRoot) {
    ...
    if (!validation.valid) {
      outputError("INVALID_WORKTREE_ROOT", validation.error, {
        suggestion:
          "Provide a valid directory path that exists or can be created",
      });
    }
...
function getDirtyStateDetails() {
  ...
  const staged = lines.filter(
    (l) => l.startsWith("A ") || l.startsWith("M ") || l.startsWith("D "),
  ).length;
...
function branchExists(branchName, cwd) {
  const local = git(`show-ref --verify --quiet refs/heads/${branchName}`, {
    silent: true,
    cwd,
  });
...
function cmdCreate() {
  ...
  if (isMonorepo) {
    ...
    if (!project || !feature) {
      outputError(
        "MISSING_ARGS",
        "Both project and feature are required for monorepo",
        {
          suggestion:
            "Usage: node worktree.cjs create <project> <feature> --prefix <type>",
  ...
  if (checkDirtyState()) {
    ...
    warnings.push(
      `Uncommitted changes: ${details.modified} modified, ${details.staged} staged, ${details.untracked} untracked`,
    );
  }
  ...
  if (isMonorepo) {
    ...
    if (matches.length > 1) {
      outputError(
        ...
        `Multiple projects match "${project}"`,
        {
          suggestion: "Use AskUserQuestion to let user select one",
          matchingProjects: matches.map((p) => ({
            name: p.name,
            path: p.path,
          })),
        },
    ...
    if (!fs.existsSync(workDir)) {
      outputError(
        ...
        `Project directory not found: ${workDir}`,
        {
  ...
  if (sanitizedFeature !== feature.toLowerCase().replace(/\s+/g, "-")) {
    warnings.push(
      `Feature name sanitized: "${feature}" → "${sanitizedFeature}"`,
    );
  }
  ...
  if (isBranchCheckedOut(branchName, workDir)) {
    outputError(
      ...
      {
        suggestion:
          "Use a different feature name or remove the existing worktree",
      },
    );
  }
  ...
  try {
    ...
    outputError(
      "MKDIR_FAILED",
      `Failed to create worktrees directory: ${worktreesDir}`,
      {
  ...
  if (branchStatus === "remote") {
    const fetchResult = git(`fetch origin ${branchName}`, {
      silent: true,
      cwd: workDir,
    });
    if (!fetchResult.success) {
      outputError(
        "FETCH_FAILED",
        `Failed to fetch branch from remote: ${branchName}`,
        {
  ...
  let createResult;
  if (branchStatus) {
    createResult = git(`worktree add "${worktreePath}" ${branchName}`, {
      cwd: workDir,
    });
  } else {
    createResult = git(
...
function cmdRemove() {
  ...
  if (removableMatches.length === 0) {
    outputError("WORKTREE_NOT_FOUND", `No worktree matching "${arg1}" found`, {
      ...
      availableWorktrees: worktrees
        .filter((w) => !w.path.includes(".git/"))
        .map((w) => path.basename(w.path)),
    });
  }
  ...
  if (removableMatches.length > 1) {
    outputError(
      ...
      `Multiple worktrees match "${arg1}"`,
      {
        suggestion: "Be more specific or use full path",
        matchingWorktrees: removableMatches.map((w) => ({
  ...
  // Remove worktree
  const removeResult = git(`worktree remove "${worktreePath}" --force`, {
    silent: true,
  });
  ...
  if (branchName) {
    ...
    if (deleteResult.success) {
      ...
      const forceDeleteResult = git(`branch -D "${branchName}"`, {
        silent: true,
      });
