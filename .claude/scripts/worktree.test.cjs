...
test("info detects repo type", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    ["standalone", "monorepo"].includes(json.repoType),
    "Should detect repo type",
  );
});
...
test("info detects base branch", () => {
  ...
  assert(
    ["dev", "develop", "main", "master"].includes(json.baseBranch),
    "Should be valid branch",
  );
});
...
test("info detects dirty state", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    typeof json.dirtyState === "boolean",
    "Should have dirtyState boolean",
  );
});
...
test("create requires feature name", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === "MISSING_FEATURE",
    "Should have MISSING_FEATURE error",
  );
});
...
test("create dry-run shows correct branch name", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.branch === "fix/my-feature",
    "Branch should be fix/my-feature",
  );
});
...
test("create sanitizes feature name - spaces", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.branch.includes("my-cool-feature"),
...
test("create respects --prefix flag", () => {
  ...
  for (const prefix of prefixes) {
    const result = run(
      `create test-${prefix} --prefix ${prefix} --dry-run --json`,
    );
    const json = assertJSON(result.output);
    assert(
      json.wouldCreate.branch.startsWith(`${prefix}/`),
      `Should use ${prefix} prefix`,
    );
  }
});
...
test("create shows worktree path", () => {
  ...
  assert(
    json.wouldCreate.worktreePath.includes("worktrees"),
    "Path should include worktrees dir",
  );
});
...
test("create in monorepo with project works", () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  const result = run(
    "create engineer test-mono --prefix feat --dry-run --json",
    { cwd: MONOREPO_DIR },
  );
  assert(result.success, "Should succeed with project");
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.project === "claudekit-engineer",
...
test("create detects invalid project", () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  const result = run("create nonexistent test-invalid --json", {
    cwd: MONOREPO_DIR,
  });
  assert(!result.success, "Should fail with invalid project");
  const json = assertJSON(result.output);
  assert(
    json.error.code === "PROJECT_NOT_FOUND",
    "Should have PROJECT_NOT_FOUND error",
  );
});
...
test("remove requires worktree name", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === "MISSING_WORKTREE",
    "Should have MISSING_WORKTREE error",
  );
});
...
test("remove handles not found", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === "WORKTREE_NOT_FOUND",
    "Should have WORKTREE_NOT_FOUND error",
  );
});
...
test("remove error includes available worktrees", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    Array.isArray(json.error.availableWorktrees),
...
test("info shows worktreeRoot and worktreeRootSource", () => {
  ...
  assert(
    typeof json.worktreeRoot === "string",
    "worktreeRoot should be string",
  );
  assert(
    json.worktreeRoot.includes("worktrees"),
    "worktreeRoot should include worktrees",
  );
});
...
test("create --worktree-root overrides default location", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.worktreePath.startsWith(customRoot),
    "Path should use custom root",
  );
  assert(
...
test("create --worktree-root with relative path resolves to absolute", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    path.isAbsolute(json.wouldCreate.worktreePath),
...
test("superproject detection in submodule", () => {
  ...
  assert(
    json.worktreeRootSource.includes("superproject") ||
      json.worktreeRootSource === "monorepo root",
...
test("WORKTREE_ROOT env var overrides detection", () => {
  ...
  try {
    ...
    assert(
      json.wouldCreate.worktreePath.startsWith(envRoot),
      "Should use env var root",
    );
    assert(
...
test("create --worktree-root validates path existence", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === "INVALID_WORKTREE_ROOT",
    "Should have INVALID_WORKTREE_ROOT error",
  );
});
...
test("unknown command returns error", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === "UNKNOWN_COMMAND",
    "Should have UNKNOWN_COMMAND error",
  );
});
...
test("create handles mixed case camelCase", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.branch.includes("mynewfeature"),
    "Should be lowercase",
  );
});
...
test("create handles path with spaces via --worktree-root", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.worktreePath.includes("my worktree dir"),
...
test("create validates file path as worktree root", () => {
  ...
  const result = run(
    'create test-file --prefix feat --json --worktree-root "/etc/passwd"',
  );
  assert(!result.success, "Should fail when path is file");
  const json = assertJSON(result.output);
  assert(
    json.error.code === "INVALID_WORKTREE_ROOT",
    "Should have INVALID_WORKTREE_ROOT",
  );
  assert(
    json.error.message.includes("not a directory"),
...
test("create handles current directory as worktree root", () => {
  const result = run(
    'create test-current --prefix feat --dry-run --json --worktree-root "."',
  );
  assert(result.success, "Should succeed");
  const json = assertJSON(result.output);
  assert(
...
test("create handles invalid prefix gracefully", () => {
  ...
  const result = run(
    "create test-custom-prefix --prefix custom --dry-run --json",
  );
  assert(result.success, "Should succeed");
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.branch.startsWith("custom/"),
...
test("create with partial project match in monorepo", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.wouldCreate.project === "claudekit-cli",
    "Should find claudekit-cli",
  );
});
...
test("create detects multiple project matches", () => {
  ...
  const result = run("create claudekit test-multi --prefix feat --json", {
    cwd: MONOREPO_DIR,
  });
  ...
  assert(
    json.error.matchingProjects.length > 1,
    "Should list multiple matches",
  );
});
...
test("remove matches by full path", () => {
  ...
  if (removable) {
    ...
    const json = assertJSON(result.output);
    assert(
      json.wouldRemove.worktreePath === removable.path,
...
test("remove matches by branch name", () => {
  const listResult = run("list --json");
  const listJson = assertJSON(listResult.output);
  const removable = listJson.worktrees.find(
    (w) => w.branch && !w.path.includes(".git/"),
  );
...
test("info provides dirty state details", () => {
  ...
  if (json.dirtyState) {
    ...
    assert(
      typeof json.dirtyDetails.modified === "number",
      "Should have modified count",
    );
    assert(
      typeof json.dirtyDetails.staged === "number",
      "Should have staged count",
    );
    assert(
      typeof json.dirtyDetails.untracked === "number",
      "Should have untracked count",
    );
  }
});
...
test("info text output includes all sections", () => {
  ...
  assert(
    result.output.includes("Worktree location:"),
    "Should have worktree location",
  );
});
...
test("create dry-run shows if branch exists", () => {
  ...
  const json = assertJSON(result.output);
  assert(
    typeof json.wouldCreate.branchExists === "boolean",
    "Should indicate branch existence",
  );
});
...
test("multiple info calls return consistent data", () => {
  ...
  assert(
    json1.baseBranch === json2.baseBranch,
    "Base branch should be consistent",
  );
  assert(
    json1.worktreeRoot === json2.worktreeRoot,
    "Worktree root should be consistent",
  );
});
...
test("list returns consistent worktree count", () => {
  ...
  const json2 = assertJSON(result2.output);
  assert(
    json1.worktrees.length === json2.worktrees.length,
    "Worktree count should be consistent",
  );
});
...
test("scenario: new user creates first worktree", () => {
  ...
  const createResult = run(
    "create add-login-feature --prefix feat --dry-run --json",
  );
  ...
  assert(
    create.wouldCreate.baseBranch === info.baseBranch,
    "Should use detected base branch",
  );
});
...
test("scenario: user fixes bug in submodule", () => {
  ...
  const result = run("create fix-auth-bug --prefix fix --dry-run --json", {
    cwd: submodulePath,
  });
...
test("scenario: user cleans up old worktrees", () => {
  ...
  assert(
    remove.error.availableWorktrees,
    "Should show available worktrees for cleanup",
  );
});
...
test("scenario: user with WORKTREE_ROOT env var", () => {
  ...
  try {
    ...
    assert(
      json.worktreeRootSource === "WORKTREE_ROOT env",
