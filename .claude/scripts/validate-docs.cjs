...
function validate(docsDir, srcDirs, projectRoot) {
  ...
  const hasIssues =
    issues.codeRefs.length || issues.links.length || issues.envVars.length;
  ...
  if (stats.codeRefsValid > 0)
    console.log(`✅ ${stats.codeRefsValid} code references validated`);
  if (stats.linksValid > 0)
    console.log(`✅ ${stats.linksValid} internal links working`);
  if (stats.envVarsValid > 0)
    console.log(`✅ ${stats.envVarsValid} config keys confirmed`);
  if (
    stats.codeRefsValid === 0 &&
    stats.linksValid === 0 &&
    stats.envVarsValid === 0
  ) {
